/*! JLB
Copyright Ishtmeet Singh © All rights reserved
MIT License
*/
(function (global) {
  ...
  function scriptTagExists(uri) {
    ...
    while ((script = docScripts[i++])) {
      if (
        typeof script.src === sSTRING &&
        uri === canonicalScriptURI(script.src)
      )
        return bTRUE;
    }
    return bFALSE;
  ...
  function engine(opts) {
    ...
    function loadScript(o) {
      ...
      if (
        !allowDup &&
        ((typeof all_scripts[src_uri] !== sUNDEF &&
          all_scripts[src_uri] !== null) ||
    ...
    publicAPI = {
      script: function () {
        var args = serializeArgs(arguments),
          ...
          i;
        for (i = 0; i < args.length; i++) {
          if (typeof args[i] === sSTRING)
            args[i] = { src: canonicalScriptURI(args[i]) };
          else if (typeof args[i].src !== sUNDEF)
            args[i].src = canonicalScriptURI(args[i].src);
        }

        if (sync_script_loading) {
          // handle without core LABjs since we're sync loading
          use_engine = publicAPI;
          for (i = 0; i < args.length; i++) loadScript(args[i]);
        } else {
          // pass off to core LABjs to handle async loading
          if (orig_engine === null)
            orig_engine = orig_$LAB.setOptions(opts.pubMap);
      ...
      wait: function (func) {
        ...
        if (sync_script_loading) {
          // execute immediately since we're sync loading
          use_engine = publicAPI;
          fSETTIMEOUT(func, 0);
        } else {
          // pass off to core LABjs to handle since we're async loading
          if (orig_engine === null)
            orig_engine = orig_$LAB.setOptions(opts.pubMap);
  ...
  function processOpts(opts) {
    ...
    for (k in allOpts) {
      if (
        allOpts.hasOwnProperty(k) &&
        typeof global_defs[allOpts[k]] !== sUNDEF
      )
        newOpts[allOpts[k]] =
          opts && typeof opts[k] !== sUNDEF ? opts[k] : global_defs[allOpts[k]];
    }
    for (k in boolOpts) {
      // normalize bool props to actual boolean values if not already
      if (boolOpts.hasOwnProperty(k))
        newOpts[boolOpts[k]] = !!newOpts[boolOpts[k]];
    }
    newOpts.preload = newOpts.cache = newOpts.order = newOpts.xhr = bFALSE; // don't use any preloading techniques if working in file:///
    newOpts.which =
      newOpts.which === sHEAD || newOpts.which === sBODY
        ? newOpts.which
        : sHEAD;
    newOpts.pubMap = {};
    for (k in allOpts) {
  ...
  global.$LAB = {
    setGlobalDefaults: function (gdefs) {
      // intentionally does not return an "engine" instance -- must call as stand-alone function call on $LAB
